        ...
        <TabsContent value="badges" className="space-y-6">
          <Card className="bg-[#111827] border-[#2D3748]">
            <CardHeader>
              ...
              <CardDescription>Collect badges by completing activities on Prior Protocol</CardDescription>
            </CardHeader>
            <CardContent>
              {badgesLoading ? (
                <div className="h-40 flex items-center justify-center">
                  <div className="w-8 h-8 border-t-2 border-b-2 border-blue-500 rounded-full animate-spin"></div>
                  <p className="ml-2 text-[#A0AEC0]">Loading your badges...</p>
                </div>
              ) : (
                <div className="space-y-6">
                  ...
                  <div className="flex flex-col space-y-2">
                    <div className="flex justify-between">
                      <span className="text-sm">Badge Completion</span>
                      <span className="text-sm">{unlockedBadges.length}/{allPossibleBadges}</span>
                    </div>
                    ...
                    {unlockedBadges.length > 0 ? (
                      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
                        {unlockedBadges.map(badgeId => {
                          ...
                          return (
                            <BadgeCard key={badgeId} badgeId={badgeId} showRarity />
                          );
                        })}
                      </div>
                    ) : (
                      <div className="text-center py-8 px-4 border border-dashed border-[#2D3748] rounded-lg">
                        <i className="far fa-star text-3xl text-[#A0AEC0] mb-3"></i>
                        <h4 className="text-lg font-medium mb-2">No badges yet</h4>
                        <p className="text-[#A0AEC0] text-sm">
                          Complete quests, use the faucet, and participate in the protocol to earn badges.
                        </p>
                      </div>
                    )}
                  </div>
                  
                  {/* Special NFT Pioneer badge card */}
                  <div className="border-t border-[#2D3748] pt-6 mt-8">
                    <h3 className="text-lg font-semibold mb-4">Special Badges</h3>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      <PioneerBadgeCard />
                      
                      {/* Add other special badges here */}
                    </div>
                  </div>
                  ...
                  {lockedBadges.length > 0 && (
                    <div className="border-t border-[#2D3748] pt-6 mt-8">
                      <h3 className="text-lg font-semibold mb-4 flex items-center">
                        <FaLock className="text-sm mr-2 text-[#A0AEC0]" />
                        Locked Badges
                      </h3>
                      ...
                      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
                        {lockedBadges.map(badgeId => (
                          <div 
                            key={badgeId}
                            className="relative p-4 border border-[#2D3748] rounded-lg bg-[#1A1F2B] opacity-60 filter grayscale"
                          >
                            <BadgeCard badgeId={badgeId} showRarity={false} />
                            <div className="absolute inset-0 flex items-center justify-center">
                              <div className="bg-[#000000] bg-opacity-30 p-2 rounded-full">
                                <FaLock className="text-lg text-[#A0AEC0]" />
                              </div>
                            </div>
                          </div>
                        ))}
