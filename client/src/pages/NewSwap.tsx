...
// Define custom Provider class compatible with ethers v6
class CustomProvider implements Provider {
  ...
  async getSigner(): Promise<Signer> {
    return new CustomSigner(this.provider);
  }
}

// Define custom Signer class
...
// Helper functions to bridge older ethers methods to v6
const formatEther = (value: ethers.BigNumberish): string => {
...
const parseUnits = (value: string, decimals: number): ethers.BigNumber => {
...
const NewSwap: React.FC = () => {
  ...
  const [tokenData, setTokenData] = useState<Record<string, TokenInfo>>(TOKENS);
  ...
  const checkAllowanceSupport = async (provider: ethers.BrowserProvider, walletAddress: string): Promise<void> => {
    try {
  ...
  // Load token metadata
  const loadTokenData = async (provider: ethers.BrowserProvider): Promise<void> => {
    try {
  ...
  // Load balances
  const loadBalances = async (walletAddress: string): Promise<void> => {
    try {
      const provider = new ethers.BrowserProvider(window.ethereum!);
  ...
  const approveTokens = async (): Promise<boolean> => {
    if (!account || !fromAmount) return false;
  ...
  const handleToTokenSelect = useCallback((token: string) => {
    setToToken(token);
  ...
  return (
    <div className="min-h-screen bg-[#0B1118] py-12 px-4 sm:px-6 lg:px-8">
      {notification && (
      ...
      <div className="max-w-md mx-auto space-y-6">
        {/* Header Card */}
        <div className="bg-[#141D29] p-8 rounded-2xl border border-[#2D3748] shadow-xl hover:shadow-[#FF6B00]/10 transition-all duration-300">
          <div className="text-center">
            <h1 className="text-3xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-[#FF6B00] to-[#FF9900] mb-2">
              PRIOR SWAP
            </h1>
            <p className="text-gray-300 mb-6">Swap PRIOR to other tokens on Base Sepolia</p>
            
            {!account ? (
              <button 
                onClick={connectWallet} 
                className="w-full bg-gradient-to-r from-[#FF6B00] to-[#FF9900] hover:from-[#FF5500] hover:to-[#FF8800] text-white font-bold py-3 px-6 rounded-xl transition-all duration-300 transform hover:scale-[1.02]"
              >
                Connect Wallet
              </button>
            ) : (
        ...
        {account && (
          <>
            {/* Wallet Info Card */}
            <div className="bg-[#141D29] p-6 rounded-2xl border border-[#2D3748] shadow-xl hover:shadow-[#FF6B00]/10 transition-all duration-300">
              <div className="mb-6">
                <h2 className="text-2xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-[#FF6B00] to-[#FF9900]">
                  Wallet Information
                </h2>
                <div className="w-12 h-1 bg-gradient-to-r from-[#FF6B00] to-[#FF9900] mt-2 rounded-full"></div>
              </div>
              
              <div className="space-y-4">
                <div>
                  <p className="text-gray-400 mb-2">Wallet Address</p>
                  <div className="flex items-center justify-between bg-[#0B1118] p-3 rounded-xl border border-[#2D3748]">
                    ...
                    <button 
                      ...
                      className="text-gray-400 hover:text-[#FF6B00] transition-all duration-200 ml-2 p-1 rounded-lg hover:bg-gray-600/50"
                ...
                <div className="grid grid-cols-2 gap-4">
                  {[
                    ...
                    { label: `${tokenData.PRIOR.symbol} Balance`, value: `${parseFloat(balances.PRIOR).toFixed(2)} ${tokenData.PRIOR.symbol}`, color: "from-[#FF6B00] to-[#FF9900]" },
                    ...
                    <div key={index} className={`bg-gradient-to-br ${item.color}/20 p-3 rounded-xl border border-[#2D3748]`}>
            ...
            <div className="bg-[#141D29] p-6 rounded-2xl border border-[#2D3748] shadow-xl hover:shadow-[#FF6B00]/10 transition-all duration-300">
              ...
              <div className="space-y-4">
                ...
                <div className="bg-[#0B1118] p-4 rounded-xl border border-[#2D3748]">
                  ...
                  <div className="flex items-center">
                    ...
                    <div className="relative">
                      <button 
                        ...
                        className="flex items-center gap-1 bg-[#111827] hover:bg-[#1A202C] px-3 py-1 rounded-lg ml-2 transition-all duration-200"
                      >
                      ...
                      {showToDropdown && (
                        <div className="absolute right-0 mt-2 w-32 bg-[#1A202C] rounded-lg shadow-lg z-10 border border-[#2D3748]">
                          {["USDC", "USDT"].map(token => (
                            <button
                              key={token}
                              onClick={() => handleToTokenSelect(token)}
                              className="w-full text-left px-4 py-2 hover:bg-[#2D3748] rounded-lg flex items-center gap-2"
                            >
                ...
                <div className="pt-2">
                  {chainId !== 84532 ? (
                    ...
                    <button 
                      disabled
                      className="w-full bg-[#0B1118] text-gray-400 font-bold py-3 px-6 rounded-xl border border-[#2D3748]"
                    >
                    ...
                    <>
                      ...
                      <button
                        ...
                        className={`w-full ${isSwapping ? 'bg-[#2D3748]' : 'bg-gradient-to-r from-[#FF6B00] to-[#FF9900] hover:from-[#FF5500] hover:to-[#FF8800]'} text-white font-bold py-3 px-6 rounded-xl transition-all duration-300 transform hover:scale-[1.02]`}
                      >
            ...
            <div className="bg-[#141D29] p-6 rounded-2xl border border-[#2D3748] shadow-xl hover:shadow-[#FF6B00]/10 transition-all duration-300">
              ...
              <div className="space-y-3">
                {[
                  ...
                  <div key={index} className="flex justify-between items-center py-2 border-b border-[#2D3748] last:border-0">
